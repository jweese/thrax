...
public class ContextPhrase {
	...
	public void addFeature(String feature_name, float feature_value) {
		Text feature_text = new Text(feature_name);
		Writable current_value = features.get(feature_text);
		if (current_value != null)
			features.put(feature_text, new FloatWritable(((FloatWritable) current_value).get() + feature_value));
		else
			features.put(feature_text, new FloatWritable(feature_value));
	}
